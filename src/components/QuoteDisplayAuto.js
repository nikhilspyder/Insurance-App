...
const QuoteDisplayAuto = () => {
    ...
    useEffect(() => {
        const fetchQuotes = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/api/autoInsurance/quotes/${requestId}`);
                setQuotes(response.data.quotes);
            } catch (error) {
                console.error('Error fetching quotes:', error);
            }
...
export default QuoteDisplayAuto;
